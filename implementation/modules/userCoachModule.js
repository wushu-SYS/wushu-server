...
async function insertNewCoachDB(trans, users, coachDetails, i) {
    return trans.query({
        sql: ` INSERT INTO user_Coach (id, firstname, lastname, phone, email, birthdate, address, sportclub,photo)
...
async function getCoachProfileById(id) {
    let ans = new Object();
    await dbConnection.query({
        sql: `Select * from user_Coach Where id= :idCoach`,
        params: {
            idCoach: id
        }
    }).then(function (results) {
